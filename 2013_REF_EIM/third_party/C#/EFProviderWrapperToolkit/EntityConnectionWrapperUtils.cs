...
namespace EFProviderWrapperToolkit
{
    ...
    public static class EntityConnectionWrapperUtils
    {
        ...
        private static void ParseResources(string resPath, List<XElement> csdl, List<XElement> ssdl, List<XElement> msl)
        {
            ...
            foreach (Assembly asm in assembliesToConsider.Where(asm => !IsEcmaAssembly(asm) && !IsSystemAssembly(asm) && !IsDynamicAssembly(asm)))
            {
        ...
        {
            string name = asm.GetType().FullName;
            //Console.WriteLine(name);
            // Dynamic assemblies should be excluded
            if (name == "System.Reflection.Emit.InternalAssemblyBuilder")
            {
                return true;
            }
            else
